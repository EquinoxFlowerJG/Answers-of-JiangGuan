...
obj.push(1);  // 2 push方法的返回值是数组的长度，但有重复属性名，此处为修改了2: 1（但是没有变量接收返回值，故此步不输出任何值）
obj.push(2);  // 2 有重复的属性名，此处为修改属性3的属性值（但是没有变量接收返回值，故此步不输出任何值）
console.log(obj);  // {2: 1, 3: 2, length: 2, push: Array.prototype.push}
```
4. 阿里超经典面试题
```js
function Foo() {
